...
class FrontendController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['categories'] = Doctor::whereActive(true)->limit(6)->select('doctor_image','name_en')->get();  // Doctor = categories
        $data['services'] = Hospital::latest()->whereActive(true)->select('image','name_en')->get();  // hospital = service
        $data['departments'] = BisesoggoCategory::whereActive(true)->limit(3)->select('image','name_en','excerpt_en')->get();
        $data['newses'] = BlogPost::whereActive(true)->limit(3)->get();
        $data['sliders'] = FrontSlider::whereActive(true)->select('featured_image','title')->get();
   ...
   public function selectGetDoctor(Request $request){
    ...
    public function shasthoseba(Request $request)
    {
        ...
        return view("frontend.home.shasthoseba", compact('products', 'categories'));
    }
